...
export const getAllActivities = async (req, res) => {
   try {
      ...
      const pendingApprovalActivities = activities.filter((row) => {
         return (
            row.approvalStatus === "Pending Approval" && row.isDraft === false
         );
      });
      res.status(200).json({
         publishedActivities,
...
export const getPreSignedImgs = async (req, res) => {
   try {
      ...
      if (foundActivity.linkedVendor.companyLogo) {
         vendorProfile = await s3GetImages(
...
export const approveActivity = async (req, res) => {
   ...
   try {
      ...
      for (const ruleId of savedActivity.activityPricingRules) {
         try {
            const rule =
               await ActivityPricingRulesModel.findById(ruleId).session(
                  session
               );
            if (!rule) {
            ...
            const updatedRule =
               await ActivityPricingRulesModel.findByIdAndUpdate(
                  ruleId,
                  { clientPrice },
                  { new: true, session }
               );
...
export const getMinAndMaxPricePerPax = async (req, res) => {
   try {
      ...
      const minPrice = Math.min(
         ...pricingRules.map((rule) => rule.clientPrice)
      );
